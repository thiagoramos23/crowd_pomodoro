defmodule CrowdPomodoroWeb.TaskLiveTest do
  use CrowdPomodoroWeb.ConnCase

  import Phoenix.LiveViewTest

  alias CrowdPomodoro.Pomodoros

  @create_attrs %{description: "some description", status: "some status", time: ~N[2010-04-17 14:00:00], time_as_string: "some time_as_string"}
  @update_attrs %{description: "some updated description", status: "some updated status", time: ~N[2011-05-18 15:01:01], time_as_string: "some updated time_as_string"}
  @invalid_attrs %{description: nil, status: nil, time: nil, time_as_string: nil}

  defp fixture(:task) do
    {:ok, task} = Pomodoros.create_task(@create_attrs)
    task
  end
